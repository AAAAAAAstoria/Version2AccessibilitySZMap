...
(function () {
  ...
  var CATEGORY_COLORS = { '医院': '#e2554d', '大学': '#2f80ed', '酒店': '#9333ea', '商场': '#ea8a0c', '公园': '#16a34a' };
  var CATEGORY_STYLE = { '医院': 'hospital', '大学': 'university', '酒店': 'hotel', '商场': 'mall', '公园': 'park' };
  ...
  /* ---------------- 部署环境检测 ---------------- */
  window.checkDeployEnv = function (useProxy) {
    if (!useProxy) return;
    var host = location.hostname;
    var isLocal = /^(localhost|127\.0\.0\.1|0\.0\.0\.0|\[::1\])$/.test(host) || location.protocol === 'file:' || host === '';
    if (!isLocal) {
      var b = document.getElementById('deploy-banner');
